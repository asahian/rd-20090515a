...
public class Level {
    ...
    public Level(int width, int height, int depth) {
        ...
        this.chunkAmountX = width / CHUNK_SIZE;
        this.chunkAmountY = depth / CHUNK_SIZE;
        this.chunkAmountZ = height / CHUNK_SIZE;
        ...
        for (int x = 0; x < this.chunkAmountX; x++) {
            for (int y = 0; y < this.chunkAmountY; y++) {
                for (int z = 0; z < this.chunkAmountZ; z++) {
                    ...
                    maxChunkX = Math.min(width, maxChunkX);
                    maxChunkY = Math.min(depth, maxChunkY);
                    maxChunkZ = Math.min(height, maxChunkZ);

                    // Create chunk based on bounds
                    Chunk chunk = new Chunk(this, minChunkX, minChunkY, minChunkZ, maxChunkX, maxChunkY, maxChunkZ);
